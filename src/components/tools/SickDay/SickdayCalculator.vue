<template>
  <div>
    ...
    <input v-model="workHours" name="workHours" type="number" step="0.01" />
    <div class="table">
      ...
      <div class="tableBody">
        <sickday
          ...
          :accumulated-hours="
            sickdays
              .slice(0, index + 1)
              .reduce((total, next) => total + Number(next.hours), Number(0))
          "
        ></sickday>
    ...
    <div>
      Total Hours:
      {{ sickdays.reduce((a, b) => a + Number(b.hours), Number(0)) }}
    </div>
  </div>
</template>

<script lang="ts">
import Sickday from "./Sickday.vue";
import CreateSickday from "./CreateSickday.vue";
